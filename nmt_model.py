...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, encode_output = self.encoder(X)
        last_hidden, last_cell = encode_output
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Yt_list = torch.split(Y,1)
        for Yt in Yt_list:
            Ybar_t = torch.cat([torch.squeeze(Yt),o_prev],axis=1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = torch.bmm(F.softmax(e_t,dim=-1).unsqueeze(dim=1),enc_hiddens).squeeze(1)
